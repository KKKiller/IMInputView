...
@property (assign, nonatomic) NSInteger selectPicsNum;
@end
...
- (void)setupView
{
    ...
    NSString *picCount = [NSString stringWithFormat:@"(%zd/9)",App_Delegate.selectedPicNum];
    UIBarButtonItem *item1 = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedStringFromTable(picCount, @"DNImagePicker", picCount) style:UIBarButtonItemStylePlain target:self action:@selector(previewAction)];
...
- (void)removeAssetsObject:(ALAsset *)asset
{
    if ([self assetIsSelected:asset]) {
        ...
        App_Delegate.selectedPicNum = App_Delegate.selectedPicNum - 1;
        [self setPicNum];
        NSLog(@"%@",[NSString stringWithFormat:@"%zd",App_Delegate.selectedPicNum]);
    }
}
...
- (BOOL)seletedAssets:(ALAsset *)asset
{
    ...
    if (self.selectedAssetsArray.count >= App_Delegate.maxPicNum) {
//        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedStringFromTable(@"alertTitle", @"DNImagePicker", nil) message:NSLocalizedStringFromTable(@"alertContent", @"DNImagePicker", nil) delegate:self cancelButtonTitle:NSLocalizedStringFromTable(@"alertButton", @"DNImagePicker", nil) otherButtonTitles:nil, nil];
        NSString *message = [NSString stringWithFormat:@"最多可选择%zd张图片",App_Delegate.maxPicNum];
        UIAlertView *alerts = [[UIAlertView alloc]initWithTitle:@"提示" message:message delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
        [alerts show];
...
- (void)backButtonAction {
    App_Delegate.selectedPicNum = self.selectPicsNum;
    App_Delegate.maxPicNum = 9 - self.selectPicsNum;
